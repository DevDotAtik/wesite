...
export default function AuthForm({ mode }: AuthFormProps) {
  ...
  function handleSocialLogin(provider: string) {
    toast.info(`${provider} sign-in isn't connected yet. Use Google or email.`);
  }

  function handleGoogleFallbackClick() {
    if (!GOOGLE_CLIENT_ID) {
      toast.error("Google sign-in needs a Client ID. See setup steps below.");
      setError("Add NEXT_PUBLIC_GOOGLE_CLIENT_ID to your .env.local to enable Google sign-in.");
      return;
    }
          ...
          <div className="mb-5">
            ...
            {!googleReady ? (
              <button
                ...
                className="nb-btn nb-btn-surface nb-btn-sm w-full text-[11px]"
              >
                {googleLoading ? (
            ...
            />
            {!GOOGLE_CLIENT_ID ? (
              <p className="mt-2 text-center text-[11px]" style={{ color: "var(--nb-muted)" }}>
                Google button activates once <code className="font-mono font-bold">NEXT_PUBLIC_GOOGLE_CLIENT_ID</code> is set.
              </p>
            ) : null}
